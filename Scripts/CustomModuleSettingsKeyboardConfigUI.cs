...
public class CustomModuleSettingsKeyboardConfigUI(EverestModule module) : ModuleSettingsKeyboardConfigUI(module)
{
	public override void Reload(int index = -1)
	{
		...
		foreach( PropertyInfo prop in properties )
		{
			...
			if( ((attribInGame = prop.GetCustomAttribute<SettingInGameAttribute>()) is not null &&
			     attribInGame.InGame != Engine.Scene is Level) || prop.GetCustomAttribute<SettingIgnoreAttribute>() != null ||
			...
			if( typeof(ButtonBinding).IsAssignableFrom(prop.PropertyType) )
			{
				if( prop.GetValue(settings) is ButtonBinding binding )
				{
					...
					name = name.DialogCleanOrNull() ??
					       (name.ToLowerInvariant().StartsWith("button") ? name.Substring(6) : name).SpacedPascalCase();
					var defaults = prop.GetCustomAttribute<DefaultButtonBindingAttribute>();
			...
			{
				if( prop.GetValue(settings) is not List<ButtonBinding> bindingMap )
				{
					continue;
				}
				...
				string name = prop.GetCustomAttribute<SettingNameAttribute>()?.Name ?? prop.Name;
				name = name.DialogCleanOrNull() ??
				       (name.ToLowerInvariant().StartsWith("buttons") ? name.Substring(7) : name).SpacedPascalCase();
				...
				for( var i = 0; i < bindingMap.Count; i++ )
				{
